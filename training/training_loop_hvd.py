...
def training_loop(
    ...
    for gpu in range(submit_config.num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            G_gpu = G if gpu == 0 and hvd.rank() == 0 else G.clone(G.name + '_shadow') # Clone if not on first GPU/worker.
            D_gpu = D if gpu == 0 and hvd.rank() == 0 else D.clone(D.name + '_shadow')
    ...
    G_opt.optimizer_class = hvd.DistributedOptimizer(G_opt.optimizer_class)
    D_opt.optimizer_class = hvd.DistributedOptimizer(D_opt.optimizer_class)
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        if cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000 or done:
            ...
            # Save snapshots (only on first worker).
            if hvd.rank() == 0: 
